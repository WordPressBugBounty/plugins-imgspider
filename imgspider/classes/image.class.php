...
class WB_IMGSPY_Image extends IMGSPY_Base
{
    ...
    public static function wb_imgspy_watermark_image()
    {
        ...
        $sub_sql = " AND NOT EXISTS(SELECT post_id FROM $db->postmeta m WHERE m.post_id=a.ID AND m.meta_key='_wb_watermark' )";
        $sub_sql .= " AND EXISTS(SELECT post_id FROM $db->postmeta m2 WHERE m2.post_id=a.ID AND m2.meta_key='_wp_attachment_metadata' )";

        $sql = "SELECT a.ID FROM $db->posts a WHERE post_type='attachment' AND post_mime_type REGEXP '^image' $sub_sql ORDER BY ID DESC LIMIT $max";
        //error_log($sql."\n",3,__DIR__.'/img.txt');
        $list = $db->get_col($sql);
    ...
    public static function recover_backup()
    {
        ...
        $db = self::db();
        $db->query("DELETE FROM $db->postmeta WHERE meta_key='_wb_watermark'");
        $wp_filesystem->rmdir($backup_base,true);
