...
class WB_IMGSPY_Conf
{
  ...
  public static function admin_notices()
  {
    ...
    if (!preg_match('#imgspider_pack#', $current_screen->base)) {
      return;
    }
    ...
    $update_url = wp_nonce_url(self_admin_url('update.php?action=upgrade-plugin&plugin=') . $plugin_file, 'upgrade-plugin_' . $plugin_file);
      ...
      echo '<a href="'.esc_url($update->url).'" target="_blank" aria-label="'.esc_attr('查看'.$plugin_data['Name'].'版本'.$update->new_version.'详情').'">查看版本'.esc_html($update->new_version).'详情</a>';
      echo '或<a href="'.esc_url($update_url).'" class="update-link" aria-label="'.esc_attr('现在更新'.$plugin_data['Name']).'">现在更新</a>。';
      echo '</p></div>';
  ...
  public static function admin_enqueue_scripts($hook)
  {
    ...
    $inline_script = 'var wb_ajaxurl="' . admin_url('admin-ajax.php') . '", 
        ...
        post_types=' . wp_json_encode($post_types, JSON_UNESCAPED_UNICODE) . ',
        post_status=' . wp_json_encode($post_status, JSON_UNESCAPED_UNICODE) . ',
        category=' . wp_json_encode($category, JSON_UNESCAPED_UNICODE) . ',
        _wb_imgspider_ajax_nonce="' . $ajax_nonce . '";';

    wp_add_inline_script('wbp-imgscrapy', $inline_script, 'before');

    wp_enqueue_style('wbs-style-imgscrapy-font', IMGSPY_URI . 'assets/font-face.css', array(), IMGSPY_VERSION);
  ...
  public static function update_cnf($opt_data)
  {
